...
<el-container>
  <el-aside  width="1000px">
    <div>
        ...
        <el-row v-if="areaData.search_id!== undefined&&areaData.search_id.length>0"> 
         <el-card v-for="(item,i) in areaData.search_id" :key="i" >
            <el-avatar  :size="70" :src="item.headimg" style="cursor: pointer" @click.native="TurnToPer(item.id)"></el-avatar>
            <div>
                <span >{{item.name}}</span>
            </div>
         </el-card>
        </el-row>
        <el-row v-if="areaData.search_user!== undefined&&areaData.search_user.length>0"> 
         <el-card v-for="(item,i) in areaData.search_user" :key="i" class="author-title reply-father">
            <el-avatar  :size="70" :src="item.headimg" style="cursor: pointer" @click.native="TurnToPer(item.id)"></el-avatar>
            <div>
                <span>{{item.name}}</span>
            </div>
...
export default {
    ...
    beforeRouteEnter(to, from, next){//后来改的
        next(vm=>{
            vm.input=vm.$route.params.input
       ...
       handleSelect(key, keyPath) {
        ...
        search() {
            ...
            else if(reg.test(this.input))
            {
                 ...
                 this.$axios({
                    method: "post",
                    url: "http://121.89.166.24:8080/search", // 接口地址
                    withCredentials: "true",
                    params: {
                    text: this.input ,  // 传接口参
                    }
                })
                    .then(response => {//后来改的
        ...
        //            this.$axios({
        //             method: "post",
        //             url: "http://localhost:8080/friend/info", // 接口地址
        //             withCredentials: "true",
        //             params: {
        //             id: i ,  // 传接口参
        //             }
        ...
        //         .then(response => {//后来改的
        //                 console.log(response.data)
        //                 this.areaData.myHeader=response.data.headImg
        //                 this.areaData.myId=response.data.id
        //                 this.areaData.myName=response.data.name
        //                 }); 
        //     }
        //     else{
        //          this.$axios({
        //             method: "post",
        //             url: "http://localhost:8080/search", // 接口地址
        //             withCredentials: "true",
        //             params: {
        //             text: this.input ,  // 传接口参
        //             }
