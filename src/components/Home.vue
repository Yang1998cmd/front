...
<el-container  >
  <el-aside  class="aside" width="1000px">
    <div>
      ...
      <div v-clickoutside="hideReplyBtn" @click="inputFocus" class="my-reply">
            ...
            <el-upload
                action="http://121.89.166.24:8080/upload/weibo"
            ...
            <div class="reply-btn-box" v-show="areaData.btnShow">
                <el-button class="reply-btn" size="medium" @click="sendComment" type="primary">发布微博</el-button>
            </div>
        </div>
        <div v-for="(item,i) in areaData.comments" :key="i" class="author-title reply-father">
            <el-avatar class="header-img" :size="40" :src="item.headImg" style="cursor: pointer" @click.native="TurnToOwn(item.id)"></el-avatar>
...
export default {
    ...
    methods: {
      ...
      onError(err, file, fileList) {
        ...
        doGetdata2()
        {
            this.timer2=setInterval(this.getData2, 50000);
        },
        firstGetData() {
            this.$axios({method: "post",
            url: "http://121.89.166.24:8080/respone/passage", // 接口地址
            withCredentials: "true",
          })
            .then(response => {//后来改的
                    console.log(response.data)
        ...
        showReplyBtn(){
            this.areaData.btnShow = true
        },
        hideReplyBtn(){
            this.areaData.btnShow = false
